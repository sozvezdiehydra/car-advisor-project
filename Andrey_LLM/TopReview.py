...
class ReviewAnalyzer:
    ...
    def get_ads_by_model(self, model: str) -> List[Tuple]:
        """Получает все объявления для указанной модели автомобиля."""
        query = """
    ...
    def extract_characteristics(self, chars: Dict) -> Dict:
        """Извлекает характеристики из JSON-характеристик."""
    ...
    def compare_characteristics(self, char1: Dict, char2: Dict) -> Dict:
        """Сравнивает две записи с характеристиками и возвращает анализ."""
        compare_prompt = """
    ...
    def process_model_ads(self, model: str):
        """Обрабатывает все объявления для указанной модели, сравнивает характеристики и формирует топ."""
    ...
    def _create_top_ads_table(self):
        """Создает таблицу для хранения топа объявлений."""
        query = """
    ...
    def _create_top_ads_json_table(self):
        """Создает таблицу для хранения топ-10 объявлений в формате JSON."""
        query = """
    ...
    def _save_comparison(self, model: str, ad1_id: int, ad2_id: int,
        ...
        """Сохраняет результат сравнения в базу данных."""
        query = """
    ...
    def _save_top_ads(self, model: str, top_ads: List[Dict], stage: str):
        """Сохраняет топ объявлений в базу данных."""
    ...
    def _save_top_ads_json(self, model: str, top_ads: List[Dict], stage: str):
        """Сохраняет топ-10 объявлений в формате JSON в базу данных."""
        delete_query = "DELETE FROM top_ads_json WHERE model = %s AND stage = %s"
        self.db.cur.execute(delete_query, (model, stage))
        self.db.conn.commit()

        # Получаем URL для каждого объявления
        top_ads_with_urls = []
        for ad in top_ads:
    ...
    def _send_to_ollama(self, prompt: str) -> str:
        """Используем метод из PromtDB для отправки запроса к Ollama."""
        return self.promt_db._send_to_ollama(prompt)

    def get_ad_characteristics(self, ad_id: int) -> Dict:
        """Получает характеристики объявления по его ID."""
